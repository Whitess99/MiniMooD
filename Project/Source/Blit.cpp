...
void Blit::updateLeakiness(double osc3LfoRate, double modAmount, bool isOscModOn, bool isNoiseModOn, double modMix)  // DA FINETUNARE
{
	// V1
	//(!isNoiseModOn) ?
	//	(
	//		
	//		isOscModOn ? (leakinessTri = 0.03f, leakiness = 0.05) : (leakinessTri = 0.02f, leakiness = 0)
	//	) :
	//	(
	//		leakiness = jmap(osc3LfoRate, 0.0f, 1000.0f, 0.0f, 0.2f),
	//		leakiness = jmin(leakiness, 0.2f),
	//		isOscModOn ? leakinessTri = 0.03f : leakinessTri = 0.02f
	//	);

	// V2
	//if (!isNoiseModOn)	// If LFO is selected
	//{
	//	leakiness = jmap(osc3LfoRate, 0.0, 100.0, 0.01, 0.15);
	//	leakiness = jmin(leakiness, 0.15);
	//}
	//else				// If noise is selected
	//	leakiness = isOscModOn ? 0.05 : 0.0;
	//
	//leakinessTri = isOscModOn ? 0.03 : 0.02;

	//leakinessTri *= modAmount;
	//leakiness *= modAmount;
...
float Blit::updateWaveform(double f, int waveform, int numOsc)
{	
	...
	switch (waveform) {
		case 0:
			...
			break;
		case 1: 
			tempSample = numOsc ? updateSawDown(f) : updateSawTri(f);
			break;
		case 2:
...
float Blit::updateSawTri(double f) {
	...
	const float triSample = updateTriangle(f);  // tri prima di saw perch� tri chiama square, che setta passedNeg, oppure mettere passNeg anche in saw
